...
## CURRENT ISSUE: Cursor Hidden (Off-Screen)

### Problem Statement

After applying `retinaScale = 2` to cursor coordinates in CursorOverlay.tsx, the cursor is now completely hidden/invisible.

### Root Cause

**Double scaling!** The Rust code in `input.rs` already applies `scale_factor` (which is 2.0 on Retina displays):

```rust
// Line 43: scale_factor = pixels_high / bounds.size.height = 2.0 on Retina
let scale_factor = display.pixels_high() as f64 / bounds.size.height;

// Lines 93-94: Already scaling to pixel coordinates
let x = rel_x * scale_factor;
let y = video_y * scale_factor;
```

Then in `CursorOverlay.tsx` we added ANOTHER 2x multiplier:

```typescript
const retinaScale = 2;
const smoothedX = position.x * retinaScale * scale + offsetX; // 2x * 2x = 4x!
```

This pushes the cursor coordinates to 4x their intended value, way outside the visible container.

### Fix

**Remove the `retinaScale` multiplier** from CursorOverlay.tsx since coordinates are already in pixel space from the Rust capture.
...
3. Open editor - cursor should now be visible
4. Cursor position should match video content
