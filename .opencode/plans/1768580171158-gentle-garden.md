...
## CURRENT ISSUE: Cursor Hidden (Off-Screen) - Debug Plan

### Problem Statement

The cursor is not visible in the editor. We've tried various retina scaling approaches but the cursor remains hidden.

### Root Cause Analysis

The cursor could be hidden due to:

1. **Coordinates too large** - pushing cursor outside `overflow-hidden` container
2. **`position` is null** - no cursor data being set
3. **`cursorImageUrl` is undefined** - cursor image not loading
4. **Scale calculation produces 0 or NaN**

### Debug & Fix Plan

#### Step 1: Add Debug Logging

Add console.log to understand what values we're working with:

**File:** `src/components/editor/CursorOverlay.tsx`

```typescript
// After calculateScale call:
...
#### Step 2: Remove Retina Scale and Test

The Rust code already applies `scale_factor` (2.0 on Retina) to coordinates.
Remove ANY additional retina scaling:

```typescript
// NO retina scaling - coordinates are already in video pixel space
const smoothedX = position.x * scale + offsetX;
const smoothedY = position.y * scale + offsetY;
```

#### Step 3: Verify Overflow Doesn't Clip

Temporarily remove `overflow-hidden` from the container div to see if cursor is being clipped:

```tsx
<div className="absolute inset-0 pointer-events-none">  {/* removed overflow-hidden */}
```

#### Step 4: Add Fallback Visibility

Always show the SVG fallback cursor to verify positioning works:

```typescript
// Force fallback cursor for debugging
const hasValidCursorImage = false; // Temporarily force fallback
```
...
| File                                      | Changes                                               |
| ----------------------------------------- | ----------------------------------------------------- |
| `src/components/editor/CursorOverlay.tsx` | Add debug logging, remove retina scale, test overflow |
...
4. Check if cursor coordinates are reasonable (within container bounds)
5. Verify cursor becomes visible

---

## PREVIOUS ISSUE: Cursor Position Still Misaligned in Editor
