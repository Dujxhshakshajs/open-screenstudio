...
## CURRENT ISSUE: Cursor Y-Axis Still Inverted in Editor

### Problem Statement

After the previous fix, the cursor overlay still appears at the wrong Y position - it's inverted. When the cursor in the video is at the bottom, the overlay cursor appears at the top.

### Root Cause Analysis

The previous fix attempted to flip the Y-axis but got the coordinate systems wrong:

| Component                  | Coordinate System    | Origin                     | Y Direction            |
| -------------------------- | -------------------- | -------------------------- | ---------------------- |
| `NSEvent::mouseLocation()` | AppKit/Cocoa         | Bottom-left of main screen | **Up** (Y=0 at bottom) |
| `CGDisplay::bounds()`      | Quartz/Core Graphics | Top-left of main screen    | **Down** (Y=0 at top)  |
| Video frames (CGImage)     | Standard image       | Top-left                   | **Down** (Y=0 at top)  |

**The bug**: The current code mixes coordinate systems:

```rust
let pos = unsafe { NSEvent::mouseLocation() };  // AppKit: Y=0 at BOTTOM

// BUG: bounds.origin is in Quartz coords (Y=0 at TOP)
// but pos.y is in AppKit coords (Y=0 at BOTTOM)
let rel_y = pos.y - display_origin_y;  // Mixing incompatible systems!

let flipped_y = display_height - rel_y;  // Double-flip for main display
```

**Trace for main display** (where `display_origin_y = 0`):

- Cursor at **bottom** of screen: `NSEvent.y ≈ 0`
- `rel_y = 0 - 0 = 0`
- `flipped_y = 1080 - 0 = 1080` → cursor shown at **top** of video (WRONG!)

- Cursor at **top** of screen: `NSEvent.y ≈ 1080`
- `rel_y = 1080 - 0 = 1080`
- `flipped_y = 1080 - 1080 = 0` → cursor shown at **bottom** of video (WRONG!)

**The Y-flip is backwards!**

### Fix Plan

The key insight is that for the **main display**:

- `NSEvent::mouseLocation().y` already gives us the distance from the bottom
- Video Y=0 is at the top, and Y increases downward
- So video_y = screen_height - NSEvent.y (simple flip)

But we need to handle the display-relative offset correctly by getting the display's origin in **AppKit coordinates**, not Quartz coordinates.

#### Approach: Use Total Screen Height for Y-Flip

For the main display, we can compute the proper Y coordinate as:

```rust
// Get the total screen height (sum of all screens in AppKit's bottom-up coords)
// For main display, the origin in NSEvent is (0, 0) at bottom-left

// NSEvent gives Y from bottom. We need Y from top.
// For single-monitor (main display): video_y = display_height - NSEvent.y

// But NSEvent Y is relative to bottom of main screen, not this display.
// For multi-monitor, we need to compute the AppKit origin of this display.

// Simplified fix for main display (display_origin_y = 0 in Quartz):
// The NSEvent Y is already from the bottom of main screen.
// video_y = main_screen_height - NSEvent.y
```

**Simpler fix**: Since we're only dealing with the main display for now, and `CGDisplay::bounds().origin` for main display is (0,0), we can just flip using the **total screen space height** in AppKit coordinates.

Actually, the simplest fix: **Don't subtract display_origin_y at all for Y** (since it's in a different coordinate system), and use the correct flip formula.

#### Step 1: Fix the Y-Coordinate Transformation
...
```rust
// For multi-monitor: get the AppKit-space origin of this display
// For now, assume main display where AppKit origin Y = 0
// NSEvent.y is distance from bottom of main screen
// Video Y should be distance from top

// Get total main screen height for the flip
// (For main display, this equals display_height)
let main_screen_height = display_height; // TODO: get from NSScreen for multi-monitor

// Convert AppKit Y (bottom-up) to video Y (top-down)
// For main display: video_y = main_screen_height - NSEvent.y
let video_y = main_screen_height - pos.y;

// Scale to pixel coordinates
let y = video_y * scale_factor;
```

For X, the current approach is correct since X coordinates work the same in both systems (left-to-right).

### Files to Modify

| File                                   | Changes                                                                      |
| -------------------------------------- | ---------------------------------------------------------------------------- |
| `src-tauri/src/capture/macos/input.rs` | Fix Y-coordinate transformation - don't mix Quartz origin with AppKit coords |

### Verification

1. `bun run tauri:dev`
2. Start a recording
3. Move cursor to **bottom** of screen, wait
4. Move cursor to **top** of screen, wait
5. Stop recording, click "Edit"
6. Verify in playback:
   - When video shows cursor at **bottom**, overlay cursor is also at **bottom**
   - When video shows cursor at **top**, overlay cursor is also at **top**
   - Cursor positions match throughout the video

---

## PREVIOUS ISSUE: Cursor Positioning Fix (PARTIALLY RESOLVED)

The previous fix added:

- display_id parameter to input tracking
- Retina scaling (scale_factor)
- Multi-monitor X offset

But got the Y-flip wrong by mixing coordinate systems.
