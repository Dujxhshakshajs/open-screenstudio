...
## CURRENT ISSUE: Cursor Positioning Wrong in Editor

### Problem Statement

Cursor positions recorded during screen capture appear in the wrong location when rendered in the editor overlay. The cursor may appear offset, at half the correct position, or vertically inverted.

### Root Cause Analysis

After analyzing the capture and playback code, **3 critical coordinate system mismatches** were identified:

#### Issue 1: Y-Axis Inversion (CRITICAL)

**Location:** `src-tauri/src/capture/macos/input.rs:50-52`

```rust
let pos = unsafe { NSEvent::mouseLocation() };
let x = pos.x;
let y = pos.y;  // PROBLEM: macOS Y-origin is at BOTTOM of screen
```

- `NSEvent::mouseLocation()` returns coordinates in macOS's **flipped coordinate system** where **Y=0 is at the BOTTOM**
- Video frames from `CGDisplay::screenshot` use standard coordinates where **Y=0 is at the TOP**
- The raw Y value is stored without transformation

**Result:** Cursor appears vertically mirrored (near top when it should be near bottom, and vice versa)

#### Issue 2: Retina Scaling Mismatch (CRITICAL)

**Location:** `src-tauri/src/capture/macos/input.rs:50-52` vs `screen.rs:298-301`

- `NSEvent::mouseLocation()` returns **logical (point) coordinates** (e.g., 1440x900 on a 2880x1800 Retina display)
- `CGDisplay::screenshot` captures at **native pixel resolution** (e.g., 2880x1800)
- Video is encoded at native resolution
- Cursor positions stored in logical coordinates without scaling

**Result:** On 2x Retina displays, cursor appears at **50% of the correct position** in both X and Y

#### Issue 3: Multi-Monitor Display Offset (HIGH)

**Location:** `src-tauri/src/capture/macos/input.rs`

- `NSEvent::mouseLocation()` returns **global screen coordinates** across all monitors
- No transformation to make coordinates relative to the captured display's origin
- If recording Display 2 which starts at X=1920 globally, cursor X positions are offset by 1920 pixels

**Result:** Cursor completely off-screen or massively offset when recording secondary displays

### Fix Plan

#### Step 1: Pass Display Info to Input Tracking

**File:** `src-tauri/src/capture/input/channel.rs`

The `InputTrackingChannel` already stores `display_id` but doesn't pass it to the input tracking thread. Update `start()` to pass display bounds and scale factor.

#### Step 2: Transform Coordinates in Rust Capture

**File:** `src-tauri/src/capture/macos/input.rs`

Update `start_input_tracking` to accept display info and transform coordinates:
...
pub fn start_input_tracking(
    ...
    unix_ms_fn: fn() -> u64,
    display_id: u32,  // NEW: pass display_id
    ...
    // 1. Make coordinates relative to display origin (fixes multi-monitor)
...
| File                                     | Changes                                                         |
| ---------------------------------------- | --------------------------------------------------------------- |
| `src-tauri/src/capture/macos/input.rs`   | Add display_id param, calculate transform, apply to coordinates |
| `src-tauri/src/capture/input/channel.rs` | Pass display_id to `start_input_tracking()`                     |

### Verification

1. `bun run tauri:dev`
2. Start a recording, move cursor to all corners of the screen
3. Stop recording, click "Edit" to open editor
4. Play back and verify:
   - Cursor in **top-left** corner of video when recorded at top-left
   - Cursor in **bottom-right** corner of video when recorded at bottom-right
   - Cursor position matches video content throughout playback
5. Test on Retina display to verify 2x scaling works
6. (Optional) Test on multi-monitor setup with secondary display
